...
3. Read in  the raw training data as variable "train.raw"
4. Name the variables in train.raw using the second column of "feats" (i.e., names(train.raw<-feats[,2])
5. Read in the activity label numbers from activity_labels.txt as variable "train.y"
6. Attach column name "Activity_labels_no" to vector "train.y"
7. Read in the  subject unique IDs from train/subject_train.txt as variable "train.subject"
8. Rename train.subject as "Subject"
9. column-bind train.subject, train.y and train.raw into new variable "train.full"
10. Repeat steps 3-6 for the test data to create final variable "test.full"
11. Row-bind train.full and test.full to create dataframe "full"
12. Read in the activity labels from "Activity_labels.txt" as variable "act"
13. Rename the columns in "act" to "Activity_labels_no","Activity_labels"
14. Map activity labels in "act" to activity label numbers in "full" by using a left join to creat full.act
15. grep search the variable names of full.act for the phrases "mean" and "std", create a vector of variable names which contain either of those called "vec"
16. Select on the "id" variable out of full.act by sub setting for columns 1,2,480,481
17. Create a mean/std only subset of full.act by selecting columns which correspond to "vec", call this "data.measure"
18. Create "data.full" by column-binding the id variables back to the measure variables
19. The final transformation groups "data.full" by Activity_label and Subject, and provides a mean of each column via the "summairse_each" function from R::dplyr
20. write the final dataframe, "data.tidy", to a text file
...
##Description
The numeric variables are a subsetof the raw dataset, retaining only those variables which measure the mean or standard deviation of a measurement. Givent hat there are 180 unique Activity/Subect combinations, the resultant dataset has 180 rows, each containing the average of the stated measurements for that subset. Units are an average of the raw measurement data, which were recorded time domain signals (prefix 't' to denote time) that were captured at a constant rate of 50 Hz and calculated in 2.56 second time interval windows (hench, Hz/2.56 seconds).
